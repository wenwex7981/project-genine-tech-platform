...
export default function BulkAIResumePublisher() {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    setLogs([]);
    addLog(`Starting bulk generation for ${totalCount} resume templates on "${topic}"...`);
    ...
    try {
      while (generatedCount < totalCount) {
        ...
        setStatusText(`Generating batch of ${countToFetch} resume templates via AI...`);
        
        // 1. Fetch AI documents
        const res = await fetch("/api/bulk-generate-resume", {
          method: "POST",
        ...
        addLog(`AI successfully generated ${templates.length} templates. Parsing and uploading...`);

        // 2. Prepare for Supabase
        const dbPayload = templates.map(t => ({
          title: t.title || "Untitled Resume Template",
        ...
        const { error } = await supabase.from("resume_templates").insert(dbPayload);
        ...
        addLog(`Inserted ${templates.length} resume templates into database. Total: ${generatedCount}/${totalCount}`);
      }

      setStatusText("Complete!");
      addLog("All resume templates have been successfully generated and published!");
      ...
      <div className="mb-6 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2">
            <Bot className="text-pink-500" /> AI Resume Template Generator
          </h1>
          <p className="text-muted-foreground text-sm">Mass generate ATS-friendly resume template posts.</p>
        </div>
      </div>
          ...
          <form onSubmit={handleGenerate} className="space-y-6">
            ...
            <div className="space-y-4">
              <h2 className="text-xl font-bold border-b pb-2 flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-pink-500" /> Target Role or Domain
              </h2>
              
              <div>
                <label className="text-sm font-semibold mb-1.5 block">Target (e.g., AI Engineer, Graphic Designer)</label>
                <input 
                  required 
                  placeholder="e.g. React Developer, Mechanical Fresher" 
                  value={topic} 
                  onChange={e => setTopic(e.target.value)}
                  className="w-full p-2.5 rounded-lg border bg-gray-50 focus:ring-2 focus:ring-pink-500 outline-none"
                  disabled={isGenerating}
                />
              </div>

              <div>
                <label className="text-sm font-semibold mb-1.5 block">Number of Templates to Generate</label>
                <input 
                  required 
                  type="number" 
                  min={1} 
            ...
            <Button 
              type="submit" 
              className="w-full h-12 text-lg font-bold bg-pink-600 hover:bg-pink-700 text-white shadow-lg"
              disabled={isGenerating || !topic || totalCount <= 0}
            >
              {isGenerating ? <Loader2 className="animate-spin mr-2" /> : <Sparkles className="mr-2" />}
              {isGenerating ? "Generating..." : `Generate & Publish ${totalCount} Templates`}
            </Button>
          </form>
        </div>
        ...
        <div className="bg-zinc-950 rounded-xl p-5 border border-zinc-800 shadow-2xl flex flex-col h-full min-h-[400px]">
          ...
          {progress.current > 0 && progress.current === progress.total && !isGenerating && (
            <div className="mt-4 bg-emerald-900/30 border border-emerald-800 p-3 rounded-lg text-emerald-400 text-sm flex items-center justify-center gap-2 font-bold">
              ...
              {progress.current} Templates Published!
            </div>
          )}
        </div>
